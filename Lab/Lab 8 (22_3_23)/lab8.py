class CustomSet:
    ...
    def add(self, item):
        """Adds a new item to CustomSet. Duplicate adds are ignored - they do not increase the length, but they do
        not raise an error."""
    ...
    def remove(self, item):
        ...
        if item not in self:
            raise KeyError(f"The item {item} is not in the CustomSet")
        ...
        if self._n_buckets > self._min_buckets and self._len <= self._n_buckets // 2:
            self._rehash(self._n_buckets // 2)

    def _rehash(self, new_buckets):
        """Rehashes every item from a hash table with n_buckets to one with new_buckets. new_buckets will be either
        2*n_buckets or 1/2*n_buckets, depending on whether we are rehashing up or down."""
        ...
        for bucket in self._L:
            ...
            for item in bucket:
                ...
                new_bucket = hash(item) % new_buckets
                # add that item to the correct bucket
                new_L[new_bucket].append(item)
