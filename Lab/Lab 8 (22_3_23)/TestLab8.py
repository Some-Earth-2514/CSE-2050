...
class TestCustomSet(unittest.TestCase):
    ...
    def testFindBucket(self):
        """Tests _find_bucket. Note - we normally would not test a private attribute. We test it here to help with
        this lab. Generally, we only test the public interface (non-private attributes) of a class"""
        cs = CS()
    ...
    def testSpeed(self):
        """Makes sure add/remove are O(1). Exact times will depend on OS/Hardware used, the provided numbers are
        calibrated for Gradescope."""
        ...
        self.assertLess(t_add_remove, t_max,
                        f"adding {n} items then removing {n} items took {t_add_remove:.2f} ms, which is slower than "
                        f"the required {t_max} ms")
