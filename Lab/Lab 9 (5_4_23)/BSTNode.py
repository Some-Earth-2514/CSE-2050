class BSTNode:
    ...
    def put(self, key):
        """
        Checks to see where to put the key based on if it's greater or less than the self.key
        """
        if key < self.key:
            if self.left:
                self.left.put(key)
    ...
    def pre_order(self):
        ...
        if self.left is not None:
            yield from self.left.pre_order()  # recursively go left

        if self.right is not None:
            yield from self.right.pre_order()  # recursively go right

    def post_order(self):
        if self.left is not None:
            yield from self.left.post_order()  # recursively go left

        if self.right is not None:
            yield from self.right.post_order()  # recursively go right
